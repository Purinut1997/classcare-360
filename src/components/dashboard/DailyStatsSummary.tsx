...
function formatDayOfWeekShort(date: string) {
  const d = new Date(`${date}T12:00:00+07:00`).getDay();
...
// ─── Component 1: Arc Speedometer / Radial Gauge (Mobile-Proof) ───────────────
...
function AttendanceArcSpeedometer({
  ...
  // Semi-circle arc: 180 degrees. Radius = 68. Arc length = PI * 68 ≈ 213.63
  const radius = 68;
  ...
  return (
    <article className="daily-speedo-card group/speedo">
      <header className="daily-widget-header">
        <div className="daily-widget-title">
          <span className="daily-widget-icon is-emerald group-hover/speedo:scale-110 transition-transform">
        ...
        <span className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-black border ${tone.bg}`}>
          <span className={`h-2 w-2 rounded-full ${tone.dot} animate-ping`} />
          {checkedToday ? tone.label : 'ยังไม่เช็กชื่อ'}
        </span>
      </header>

      {/* SVG Arc Gauge Container */}
      <div className="daily-speedo-visual w-full flex flex-col items-center justify-center my-1">
        <div className="w-full max-w-[240px] h-[125px] relative flex items-center justify-center">
          <svg
            viewBox="0 0 180 110"
          ...
          >
            <defs>
              <linearGradient id="speedoGradDirect" x1="0%" y1="0%" x2="100%" y2="0%">
            ...
            {/* Active Colored Arc (No filter bug on iOS Safari) */}
            {clampedPct > 0 && (
              <path
                d="M 18 92 A 68 68 0 0 1 162 92"
                fill="none"
                stroke="url(#speedoGradDirect)"
                strokeWidth="14"
                strokeLinecap="round"
            ...
            >
              <line x1="0" y1="0" x2="0" y2="-54" stroke="#0f172a" strokeWidth="3.5" strokeLinecap="round" />
              <polygon points="-4,-18 4,-18 0,-56" fill="#0f172a" />
              <circle cx="0" cy="0" r="7" fill="#0f172a" />
              <circle cx="0" cy="0" r="3" fill="#ffffff" />
            </g>

            {/* Scale Labels */}
            <text x="18" y="106" fontSize="10" fontWeight="900" fill="#94a3b8" textAnchor="middle">0%</text>
            <text x="90" y="24" fontSize="9" fontWeight="900" fill="#94a3b8" textAnchor="middle">50%</text>
            <text x="162" y="106" fontSize="10" fontWeight="900" fill="#94a3b8" textAnchor="middle">100%</text>
          </svg>
        </div>

        {/* Center Readout */}
        <div className="daily-speedo-readout mt-1 text-center">
          <div className="text-3xl sm:text-4xl font-black tracking-tight text-slate-900 leading-none">
            {clampedPct}%
          </div>
      ...
      {/* Pill Counters: 2 Columns on Mobile, 4 on Desktop */}
      <div className="daily-speedo-pills grid grid-cols-2 sm:grid-cols-4 gap-2 mt-3 pt-3 border-t border-slate-100">
        <span className="speedo-pill is-present">
          <CheckCircle2 size={13} className="shrink-0" /> มา <strong>{present}</strong>
        </span>
        <span className="speedo-pill is-late">
          <Clock size={13} className="shrink-0" /> สาย <strong>{late}</strong>
        </span>
        <span className="speedo-pill is-leave">
          <Calendar size={13} className="shrink-0" /> ลา <strong>{leave}</strong>
        </span>
        <span className="speedo-pill is-absent">
          <Users size={13} className="shrink-0" /> ขาด <strong>{absent}</strong>
        </span>
      </div>
    </article>
  );
...
// ─── Component 2: 7-Day Attendance Quality Heatmap Matrix (Mobile-Proof) ───────
...
function AttendanceQualityHeatmap({ trend, onSelectDay, selectedDay }: HeatmapProps) {
  // Always ensure last 7 days
  const points = trend.slice(-7);
  const weekAvg = calcAttendanceRate(points);

  // Compute Streak: consecutive days with >= 85% attendance
  let streak = 0;
  ...
  const getHeatLevel = (pt: AttendanceTrendPoint) => {
    if (pt.total === 0) return { bg: 'bg-slate-100 text-slate-400 border-slate-200/80', tag: 'ไม่มีข้อมูล', icon: '⚪' };
    const pct = Math.round((pt.present / pt.total) * 100);
    if (pct >= 95) return { bg: 'bg-emerald-600 text-white border-emerald-700 shadow-xs shadow-emerald-500/20', tag: 'ดีเยี่ยม', icon: '🔥' };
    if (pct >= 85) return { bg: 'bg-emerald-500 text-white border-emerald-600', tag: 'ดีมาก', icon: '✓' };
    if (pct >= 70) return { bg: 'bg-amber-400 text-slate-950 border-amber-500', tag: 'ปานกลาง', icon: '⏱' };
    return { bg: 'bg-rose-500 text-white border-rose-600', tag: 'ต้องดูแล', icon: '!' };
  };

  return (
    <article className="daily-heat-card group/heat">
      <header className="daily-widget-header">
        <div className="daily-widget-title">
          <span className="daily-widget-icon is-sky group-hover/heat:scale-110 transition-transform">
        ...
        {streak > 1 ? (
          <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-black bg-orange-50 border border-orange-200 text-orange-700 shadow-xs">
      ...
      {/* 7-Day Heat Grid: 7 equal columns fitting 100% width on any screen */}
      <div className="grid grid-cols-7 gap-1 sm:gap-2 mt-3 w-full">
        {points.map((pt, idx) => {
          ...
          return (
            <button
              ...
              className={`min-w-0 w-full flex flex-col items-center justify-between p-1 sm:p-2.5 rounded-xl sm:rounded-2xl border transition-all duration-200 active:scale-95 ${
                isSelected
                  ? 'ring-2 ring-sky-500 scale-105 shadow-md z-10'
                  : 'hover:-translate-y-0.5 hover:shadow-xs'
              } ${heat.bg}`}
              title={`${formatTrendDate(pt.date)}: มา ${pt.present}/${pt.total} คน (${rate}%)`}
            >
              {/* Short day name */}
              <span className="text-[9px] sm:text-[10px] font-black uppercase tracking-tight opacity-90 truncate w-full text-center">
                {formatDayOfWeekShort(pt.date)}
              </span>

              {/* Day number */}
              <span className="text-xs sm:text-base font-black tracking-tight my-0.5">
                {formatDayNumber(pt.date)}
              </span>

              {/* Percentage */}
              <span className="text-[9px] sm:text-[11px] font-black opacity-95">
                {pt.total > 0 ? `${rate}%` : '—'}
              </span>
            </button>
          );
      ...
      {/* Interactive Day Inspector (Click/Tap reveal) */}
      {selectedDay ? (
        <div className="daily-heat-inspector mt-3 p-3 rounded-2xl bg-sky-50/90 border border-sky-200 flex items-center justify-between gap-3 animate-in fade-in duration-200">
          <div className="flex items-center gap-2.5 min-w-0">
            <span className="grid h-8 w-8 place-items-center rounded-xl bg-white text-sky-700 shadow-xs text-xs font-black shrink-0">
              {formatDayOfWeekShort(selectedDay.date)}
            </span>
          ...
          <button
            ...
            className="text-xs font-black text-sky-700 hover:text-sky-900 shrink-0 px-2.5 py-1 rounded-lg bg-sky-100 active:scale-95"
          >
            ปิด
          </button>
        </div>
        ...
        <div className="daily-heat-legend mt-3 flex items-center justify-between text-[11px] font-bold text-slate-500 pt-1">
          <span>แตะแต่ละวันเพื่อดูตัวเลข</span>
...
function BehaviorDivergingBalance({
  ...
  return (
    <article className="daily-balance-card group/balance">
      <header className="daily-widget-header">
        <div className="daily-widget-title">
          <span className="daily-widget-icon is-purple group-hover/balance:scale-110 transition-transform">
      ...
      <div className="daily-balance-scale-wrap mt-3">
        <div className="flex items-center justify-between text-xs font-black mb-1.5">
          <span className="text-rose-600 flex items-center gap-1">
            <TrendingDown size={13} /> ปรับปรุง (-{absNeg})
          </span>
        ...
        <div className="flex items-center justify-between mt-2 text-[11px] font-bold text-slate-500">
          ...
          <span className="font-extrabold text-slate-700">{totalRecords} บันทึก</span>
      ...
      <div className="daily-balance-footer mt-3 flex items-center justify-between pt-2.5 border-t border-slate-100">
        <p className="text-xs font-semibold text-slate-600 truncate">
          {posPct >= 80 ? '🌟 พฤติกรรมเชิงบวกสูงมาก' : 'ติดตามและส่งเสริมจุดเด่น'}
        </p>
        <Link
          to="/app/dashboard?view=behavior"
          className="text-xs font-black text-purple-700 hover:text-purple-900 inline-flex items-center gap-1 group-hover/balance:translate-x-0.5 transition-transform"
        >
...
function AcademicScoreSpectrum({
  ...
  return (
    <article className="daily-academic-card group/acad">
      <header className="daily-widget-header">
        <div className="daily-widget-title">
          <span className="daily-widget-icon is-indigo group-hover/acad:scale-110 transition-transform">
      ...
      <div className="mt-3">
        ...
        <div className="flex items-center justify-between mt-2 text-[11px] font-bold text-slate-500">
          ...
          <span className="text-slate-700 font-extrabold">เกณฑ์ &ge; 60%</span>
          <Link
            to="/app/dashboard?view=scores"
            className="text-indigo-600 hover:text-indigo-800 font-black inline-flex items-center gap-1 group-hover/acad:translate-x-0.5 transition-transform"
          >
...
function SavingsDotMeter({
  ...
  return (
    <article className="daily-savings-card group/save">
      <header className="daily-widget-header">
        <div className="daily-widget-title">
          <span className="daily-widget-icon is-amber group-hover/save:scale-110 transition-transform">
      ...
      <div className="mt-3">
        ...
        <div className="flex items-center justify-between mt-2 text-[11px] font-bold text-slate-500">
          ...
          <Link
            ...
            className="text-amber-700 hover:text-amber-900 font-black inline-flex items-center gap-1 group-hover/save:translate-x-0.5 transition-transform"
          >
...
function SubjectRow({ item }: { item: SubjectAttendanceSummary }) {
  ...
  return (
    <li className="daily-subject-row hover:bg-slate-50/80 transition-colors">
      <div className="daily-subject-info">
        <span className="daily-subject-name">
          ...
          <span className="font-extrabold text-slate-800 truncate">{item.subjectName}</span>
        </span>
        ...
        <div>
          <div className="daily-stats-eyebrow">
            <span className="daily-stats-badge">
              <Zap size={11} className="text-amber-400 animate-pulse" /> DAILY INTELLIGENCE
            </span>
