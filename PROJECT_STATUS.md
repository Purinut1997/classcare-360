...
- **ศูนย์ออกแบบข้อสอบกลางภาค/ปลายภาค & รูบริก สพฐ. (AI Exam & Rubric Hub):**
  - พัฒนาระบบใส่หน่วยการเรียนรู้ได้หลายหน่วย (Multi-Unit) พร้อม AI วิเคราะห์ตัวชี้วัด สพฐ. เชื่อมโยงอัตโนมัติ และเปิดให้คุณครูป้อนหรือปรับปรุงตัวชี้วัดได้อิสระ
  - เพิ่มระบบจัดสรรโควตาข้อสอบตามตัวชี้วัด (Indicator Quota Allocation) ทั้งแบบกระจายสมดุลอัตโนมัติ (Auto-balanced) และแบบครูกำหนดจำนวนข้อต่อตัวชี้วัดเอง
  - บังคับแสดงรหัสตัวชี้วัด สพฐ. กำกับท้ายโจทย์ข้อสอบทุกข้ออย่างชัดเจน (เช่น `(ตัวชี้วัด: ค 1.1 ม.1/2)`)
  - สร้างผังการสร้างข้อสอบ (Test Blueprint / ตารางสองมิติ) คำนวณร้อยละและจำแนกตามระดับพฤติกรรมบลูม (Bloom's Taxonomy)
  - สร้างชุดข้อสอบสำหรับนักเรียน และชุดเฉลยพร้อมคำอธิบายละเอียดสำหรับครู (Teacher Key) พร้อมพิมพ์ A4
  - เพิ่มศูนย์สร้างเกณฑ์รูบริก 4 ระดับ (ดีเยี่ยม, ดี, พอใช้, ปรับปรุง) และระบบออกข้อสอบย่อยซ่อมเสริม (Remedial Quiz 1-30 ข้อ)
- **UX สมาร์ทโฟนระดับ Super-App (Smartphone Super-App Dock):**
  - แถบนำทางด้านล่าง 5 ปุ่มสไตล์ Super-App ตามหลักสรีรศาสตร์ พร้อม Center Hero FAB ขนาด 58px ยกสูงลอยตัว สำหรับเรียก "น้องแคร์ AI"
  - เพิ่ม Quick Launcher Drawer รวมทางลัด 12 หมวดหมู่หลักของระบบ พร้อมระบบค้นหาทันใจ (Live Search Filter)
- **ระบบผู้ช่วยตารางสอนอัจฉริยะ & ปฏิทินโรงเรียน (Schedule & School Calendar):**
  - ผู้ช่วย "น้องแคร์ AI" สามารถตอบคำถามตารางสอนรายวัน/รายสัปดาห์ได้โดยตรง
  - ปฏิทินโรงเรียนแสดง Dots Indicator แยกหมวดหมู่กิจกรรม และปรับปรุง Responsive Header ให้พอดีกับหน้าจอมือถือ
- **อัปเกรดคู่มือทั้งระบบ (System Manual & In-App Help Center):**
  - อัปเกรดคู่มือในแอป `HelpCenterPage.tsx` ขยายเป็นการ์ดคู่มือ 18 หัวข้อ พร้อม Category Filter Tabs แยกหมวดหมู่
  - อัปเดต `SetupGuideModal.tsx` ขั้นตอนที่ 5 ให้ครอบคลุม AI Exam Hub และน้องแคร์ AI
  - จัดทำคู่มือระบบฉบับสมบูรณ์ที่ `docs/MANUAL.md` และอัปเดต `README.md` ครบถ้วนทุกโมดูล
- ตรวจล่าสุด: `npx tsc -b` และ `npm run build` ผ่าน 100% ปราศจาก error
...
- งานที่ยังต้องทำบนระบบจริง: รัน migration ล่าสุดใน Supabase project จริง โดยเฉพาะ `0020_harden_destructive_action_rpcs.sql` แล้วทดสอบปุ่มลบห้องเรียน/workspace/ชุดคะแนนบน Cloudflare Pages production อีกครั้ง
